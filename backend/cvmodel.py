...
def score(x, y, template_x, template_y):
    ...
    while index_test < len(sorted_x):
        ...
        if sorted_x[index_test] <= check_x + 0.05 and sorted_x[index_test] >= check_x - 0.05 and sorted_y[
            index_test] <= check_y + 0.05 and sorted_y[index_test] >= check_y - 0.05:
            count += 1
            index_template += 1
